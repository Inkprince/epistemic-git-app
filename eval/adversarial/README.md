...
Cached mode is always the default, even if `LLM_API_KEY` is set. A committed cache hit executes
...
Use `--live` deliberately: it requires `LLM_API_KEY`, may send the planted source text to the
