...
Detection rates use executed traps as the denominator and separately state execution coverage. In
particular, the existing prompt-injection cache currently yields a real miss because extraction admits
the injected bleach assertion; the suite preserves that miss unless pipeline behavior actually changes.
