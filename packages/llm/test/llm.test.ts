...
import { z } from "zod";
import {
  CacheMissError, CachedLlmClient, CerebrasClient, type CompleteParams, type CompleteResult,
...
describe("Cerebras client", () => {
  it("builds an OpenAI-style request with strict json_schema and parses the response", async () => {
    const fake: FetchLike = vi.fn(async (_url, init) => {
      ...
      // assert we send the structured-output request shape
    ...
    const client = new CerebrasClient({ apiKey: "k", fetchImpl: fake });
  ...
  it("gives up on a persistent 429 after maxRetries", async () => {
    ...
    const client = new CerebrasClient({ apiKey: "k", fetchImpl: fake, maxRetries: 0, sleep: async () => {} });
  ...
  it("retries a 429 and then succeeds", async () => {
    ...
    const client = new CerebrasClient({ apiKey: "k", fetchImpl: fake, sleep: async () => {} });
